(function(){
  ...
  /* ---------- Download CV (DOCX preferred) ---------- */
  document.getElementById('downloadCv').addEventListener('click', async function(e){
    e.preventDefault();

    // Put your DOCX file inside the project, e.g.:
    //   portfolio/assets/Muhammad-Ali-Khan-Senior-Software-Engineer-CV.docx
    // Then it will be downloaded by the browser.
    const DOCX_URL = 'assets/Muhammad-Ali-Khan-Senior-Software-Engineer-CV.docx';
    const DOCX_NAME = 'Muhammad-Ali-Khan-Senior-Software-Engineer-CV.docx';

    try {
      // If the file exists, trigger a download from the served site.
      const res = await fetch(DOCX_URL, { method: 'HEAD' });
      if (res.ok) {
      ...
      // Ignore and fallback to txt generation below.
    }

    // Fallback: generate a plain-text CV.
